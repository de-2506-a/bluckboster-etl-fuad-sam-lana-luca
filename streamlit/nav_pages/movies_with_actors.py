...
def show_movies_with_actors():
    ...
    """
    st.markdown("<h1 style='text-align: center; color: #1f77b4;'>🎬 Movies and Their Actors</h1>", unsafe_allow_html=True)
    ...
    if selected_movie:
        ...
        # Movie info card
        st.markdown(f"<div style='background-color: #f0f2f6; padding: 20px; border-radius: 10px; margin: 20px 0;'><h2 style='color: #4ecdc4; margin: 0;'>🎭 {selected_movie}</h2><p style='font-size: 18px; color: #2c3e50; margin: 10px 0 0 0;'>💰 Revenue: <strong>${movie_revenue:,.2f}</strong></p></div>", unsafe_allow_html=True)
        ...
        for i, actor in enumerate(actors_df['full_name']):
            with cols[i % 3]:
                st.markdown(f"<div style='background-color: #e8f4fd; padding: 15px; border-radius: 8px; margin: 5px 0; text-align: center; border-left: 4px solid #3498db;'><p style='margin: 0; font-weight: bold; color: #2c3e50;'>🎭 {actor}</p></div>", unsafe_allow_html=True)